    ...
    class Host
      ...
      class Sources
        ...
        def open uri, text, version
          ...
          source = Lunargraph::Source.new(text, filename, version)
          open_source_hash[uri] = source
        end

        def open_from_disk uri
          source = Lunargraph::Source.load(UriHelpers.uri_to_file(uri))
          open_source_hash[uri] = source
        end
        ...
        # @return [Source]
        def find uri
          open_source_hash[uri] || raise(Lunargraph::FileNotFoundError, "Host could not find #{uri}")
        end
