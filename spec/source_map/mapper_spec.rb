describe Lunargraph::SourceMap::Mapper do
  it "creates `new` pins for `initialize` pins" do
    ...
    foo_pin = map.pins.find { |pin| pin.path == "Foo.new" }
    expect(foo_pin.return_type.tag).to eq("self")
    bar_pin = map.pins.find { |pin| pin.path == "Foo::Bar.new" }
  ...
  it "ignores include calls that are not attached to the current namespace" do
    ...
    map = Lunargraph::SourceMap.map(source)
    pins = map.pins.select { |pin| pin.is_a?(Lunargraph::Pin::Reference::Include) && pin.namespace == "Foo" }
    names = pins.map(&:name)
  ...
  it "ignores prepend calls that are not attached to the current namespace" do
    ...
    map = Lunargraph::SourceMap.map(source)
    pins = map.pins.select { |pin| pin.is_a?(Lunargraph::Pin::Reference::Prepend) && pin.namespace == "Foo" }
    names = pins.map(&:name)
    expect(names).to include("Direct")
    expect(names).not_to include("Indirect")
    expect(names).to include("Interior")
  ...
  it "ignores extend calls that are not attached to the current namespace" do
    ...
    map = Lunargraph::SourceMap.map(source)
    map.pins.find { |pin| pin.path == "Foo" }
    ...
    pins = map.pins.select { |pin| pin.is_a?(Lunargraph::Pin::Reference::Extend) && pin.namespace == "Foo" }
    names = pins.map(&:name)
    expect(names).to include("Direct")
    expect(names).not_to include("Indirect")
    expect(names).to include("Interior")
  ...
  it "sets scopes for attributes" do
    ...
    map = Lunargraph::SourceMap.map(source)
    bar1 = map.pins.find { |pin| pin.name == "bar1" }
    expect(bar1.scope).to eq(:instance)
    bar2 = map.pins.find { |pin| pin.name == "bar2" }
  ...
  it "maps method parameters" do
    ...
    pin = map.locals.find { |p| p.name == "baz" }
    expect(pin).to be_a(Lunargraph::Pin::Parameter)
    pin = map.locals.find { |p| p.name == "boo" }
    expect(pin).to be_a(Lunargraph::Pin::Parameter)
    pin = map.locals.find { |p| p.name == "key" }
    expect(pin).to be_a(Lunargraph::Pin::Parameter)
  end
  ...
  it "adds superclasses to class pins" do
    ...
    pin = map.pins.find { |p| p.is_a?(Lunargraph::Pin::Reference::Superclass) }
  ...
  it "remaps methods for module_function symbol arguments" do
    ...
    pin = map.first_pin("Functions#bar")
    expect(pin.visibility).to eq(:public)
    pin = map.pins.find { |p| p.name == "@foo" and p.context.scope == :class }
    expect(pin).to be_a(Lunargraph::Pin::InstanceVariable)
    pin = map.pins.find { |p| p.name == "@foo" and p.context.scope == :instance }
    expect(pin).to be_a(Lunargraph::Pin::InstanceVariable)
  end
  ...
  it "modifies instance variables in module functions" do
    ...
    pin = map.pins.find { |p| p.name == "@foo" and p.context.scope == :instance }
    expect(pin).to be_a(Lunargraph::Pin::InstanceVariable)
    pin = map.pins.find { |p| p.name == "@bar" and p.context.scope == :class }
    expect(pin).to be_a(Lunargraph::Pin::InstanceVariable)
    pin = map.pins.find { |p| p.name == "@bar" and p.context.scope == :instance }
    expect(pin).to be_a(Lunargraph::Pin::InstanceVariable)
  end
  ...
  it "maps class variables" do
    ...
    pin = map.pins.find { |p| p.name == "@@bar" }
    expect(pin).to be_a(Lunargraph::Pin::ClassVariable)
    pin = map.pins.find { |p| p.name == "@@baz" }
    expect(pin).to be_a(Lunargraph::Pin::ClassVariable)
  end
  ...
  it "maps local variables" do
    ...
    pin = map.locals.find { |p| p.name == "x" }
    expect(pin).to be_a(Lunargraph::Pin::LocalVariable)
  end
  ...
  it "maps global variables" do
    ...
    pin = map.pins.find { |p| p.name == "$x" }
    expect(pin).to be_a(Lunargraph::Pin::GlobalVariable)
  end
  ...
  it "maps singleton methods" do
    ...
    pin = map.first_pin("Foo.bar")
    expect(pin).to be_a(Lunargraph::Pin::Method)
  ...
  it "maps extend self" do
    ...
    pin = map.pins.find { |p| p.is_a?(Lunargraph::Pin::Reference::Extend) }
  ...
  it "maps require calls" do
    ...
    pin = map.pins.find { |p| p.is_a?(Lunargraph::Pin::Reference::Require) }
  ...
  it "maps block parameters" do
    ...
    pin = map.locals.find { |p| p.name == "y" }
    expect(pin).to be_a(Lunargraph::Pin::Parameter)
  end
  ...
  it "maps root blocks to class scope" do
    ...
    pin = smap.pins.find { |p| p.is_a?(Lunargraph::Pin::Block) }
  ...
  it "maps rebased namespaces without leading colons" do
    ...
    expect(smap.first_pin("Bar")).to be_a(Lunargraph::Pin::Namespace)
    expect(smap.first_pin("Bar#baz")).to be_a(Lunargraph::Pin::Method)
  end

  it "maps contexts of constants" do
    var = "BAR"
    smap = Lunargraph::SourceMap.load_string("#{var} = nil")
    pin = smap.pins.find { |p| p.name == var }
    expect(pin.context).to be_a(Lunargraph::ComplexType)
    smap = Lunargraph::SourceMap.load_string("#{var} ||= nil")
    pin = smap.pins.find { |p| p.name == var }
    expect(pin.context).to be_a(Lunargraph::ComplexType)
  end
  ...
  it "maps contexts of local variables" do
    var = "bar"
    smap = Lunargraph::SourceMap.load_string("#{var} = nil")
    pin = smap.locals.find { |p| p.name == var }
    expect(pin.context).to be_a(Lunargraph::ComplexType)
    smap = Lunargraph::SourceMap.load_string("#{var} ||= nil")
    pin = smap.locals.find { |p| p.name == var }
    expect(pin.context).to be_a(Lunargraph::ComplexType)
  ...
  it "maps method aliases" do
    ...
    pin = smap.pins.find { |p| p.path == "Foo#baz" }
    expect(pin).to be_a(Lunargraph::Pin::MethodAlias)
  end
  ...
  it "maps method macros" do
    ...
    pin = smap.pins.find { |p| p.path == "Foo#make" }
  ...
  it "maps method directives" do
    ...
    pin = smap.pins.find { |p| p.path == "Foo#bar" }
    expect(pin.return_type.tag).to eq("String")
  ...
  it "maps aliases to superclass methods" do
    ...
    pin = smap.pins.find { |p| p.path == "Sub#bar" }
    expect(pin).to be_a(Lunargraph::Pin::MethodAlias)
  end
  ...
  it "uses nodes for method parameter assignments" do
    ...
    pin = smap.locals.find { |p| p.name == "baz" }
    # expect(pin.assignment).to be_a(Parser::AST::Node)
    expect(Lunargraph::Parser.is_ast_node?(pin.assignment)).to be(true)
  end
  ...
  it "defers resolution of distant alias_method aliases" do
    ...
    pin = smap.pins.find { |p| p.is_a?(Lunargraph::Pin::MethodAlias) }
  ...
  it "maps rescue nodes" do
    ...
    var_pin = smap.pins.find { |p| p.name == "@y" }
  ...
  it "ignores aliases that do not map to methods or attributes" do
    expect {
      Lunargraph::SourceMap.load_string(%(
        class Foo
          xyz = String
          alias foo xyz
  ...
  it "sets visibility for symbol parameters" do
    ...
    pub = smap.pins.find { |pin| pin.path == "Foo#pub" }
    expect(pub.visibility).to eq(:public)
    bar = smap.pins.find { |pin| pin.path == "Foo#bar" }
    expect(bar.visibility).to eq(:private)
    baz = smap.pins.find { |pin| pin.path == "Foo#baz" }
    expect(baz.visibility).to eq(:public)
    pro = smap.pins.find { |pin| pin.path == "Foo#pro" }
  ...
  it "handles invalid byte sequences" do
    expect {
      Lunargraph::SourceMap.load(File.join("spec", "fixtures", "invalid_utf8.rb"))
    }.not_to raise_error
  end
  ...
  it "applies private_class_method to attributes" do
    ...
    pin = smap.pins.find { |pin| pin.path == "Foo.bar" }
    expect(pin.visibility).to eq(:private)
  end
  ...
  it "processes comments without associations" do
    ...
    expect(smap.first_pin("Foo#bar")).to be_a(Lunargraph::Pin::Method)
  end
  ...
  it "ignores attribute directives without names" do
    ...
    attrs = smap.pins.select { |pin| pin.is_a?(Lunargraph::Pin::Method) && pin.attribute? }
  ...
  it "ignores method directives without names" do
    ...
    attrs = smap.pins.select { |pin| pin.is_a?(Lunargraph::Pin::Method) }
    expect(attrs).to be_empty
  end
  ...
  it "maps multiple method directives in a class" do
    ...
    expect(bar).to be_a(Lunargraph::Pin::Base)
    baz = api_map.get_path_pins("Foo.baz").first
    expect(baz).to be_a(Lunargraph::Pin::Base)
  end
  ...
  it "processes override directives" do
    ...
    over = pins.find { |pin| pin.is_a?(Lunargraph::Pin::Reference::Override) }
  ...
  it "maps kwrestarg parameters" do
    ...
    param = locals.find { |pin| pin.is_a?(Lunargraph::Pin::Parameter) }
  ...
  it "handles private_class_method without arguments" do
    ...
    expect {
      Lunargraph::SourceMap.load_string(code, "test.rb")
    }.not_to raise_error
  end
  ...
  it 'maps Bundler.require to require "bundler/require"' do
    ...
    pin = map.pins.find { |pin| pin.is_a?(Lunargraph::Pin::Reference::Require) }
  ...
  it "sends local variables to remote class_eval receivers" do
    ...
    locals = map.locals_at(
      Lunargraph::Location.new(
        "test.rb",
        Lunargraph::Range.from_to(5, 0, 5, 0)
      )
    ).map(&:name)
  ...
  it "handles invalid byte sequences" do
    expect {
      Lunargraph::SourceMap.load("spec/fixtures/invalid_byte.rb")
    }.not_to raise_error
  end

  it "handles invalid byte sequences in stringified node comments" do
    expect {
      Lunargraph::SourceMap.load("spec/fixtures/invalid_node_comment.rb")
    }.not_to raise_error
  end
  ...
  it "parses method directives that start with multiple hashes" do
    ...
    map = Lunargraph::SourceMap.load_string(source)
