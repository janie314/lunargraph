...
describe Lunargraph::LanguageServer::Host do
  ...
  it "runs diagnostics on opened files" do
    Dir.mktmpdir do |dir|
      ...
      host.start
      host.prepare dir
      Lunargraph::LanguageServer::UriHelpers.file_to_uri(file)
  ...
  it "handles DiagnosticsErrors" do
    ...
    allow(library).to receive(:diagnose).and_raise(Lunargraph::DiagnosticsError)
  ...
  it "opens multiple folders" do
    host = described_class.new
    app1_folder = File.absolute_path("spec/fixtures/workspace_folders/folder1").tr("\\", "/")
    app2_folder = File.absolute_path("spec/fixtures/workspace_folders/folder2").tr("\\", "/")
    host.prepare(app1_folder)
    host.prepare(app2_folder)
    file1_uri = Lunargraph::LanguageServer::UriHelpers.file_to_uri("#{app1_folder}/app.rb")
    file2_uri = Lunargraph::LanguageServer::UriHelpers.file_to_uri("#{app2_folder}/app.rb")
  ...
  it "retains orphaned sources" do
    ...
    file_uri = Lunargraph::LanguageServer::UriHelpers.uri_to_file(file)
    host = described_class.new
    host.prepare(dir)
  ...
  it "rescues runtime errors from messages" do
    host = described_class.new
    message_class = Class.new(Lunargraph::LanguageServer::Message::Base) do
      def process
        raise "Always raise an error from this message"
      end
    end
    Lunargraph::LanguageServer::Message.register("raiseRuntimeError", message_class)
    expect {
      host.receive({
        "id" => 1,
