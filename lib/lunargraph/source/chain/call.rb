    ...
    class Chain
      class Call < Link
          ...
          result = pins.map do |p|
            ...
            overloads.each do |ol|
              ...
              arguments.each_with_index do |arg, idx|
                ...
                next if param.return_type.undefined? || atype.name == param.return_type.name || api_map.super_and_sub?(
                  param.return_type.name, atype.name
                )
                match = false
                break
              end
              if match
                ...
                break if type
                if ol.return_type.defined?
                  type = with_params(ol.return_type.self_to(context.to_s), context).qualify(api_map,
                                                                                            context.namespace)
                end
                type ||= ComplexType::UNDEFINED
              end
            ...
            type = extra_return_type(p.docstring, context)
            if type
              next Lunargraph::Pin::Method.new(
                location: p.location,
                closure: p.closure,
                name: p.name,
                comments: "@return [#{context.subtypes.first}]",
                scope: p.scope,
        ...
        def inner_process_macro pin, macro, api_map, _context, locals
          ...
          docstring = Lunargraph::Source.parse_docstring(txt).to_docstring
          tag = docstring.tag(:return)
          return Pin::ProxyType.anonymous(ComplexType.try_parse(*tag.types)) unless tag.nil? || tag.types.nil?
          Pin::ProxyType.anonymous(ComplexType::UNDEFINED)
        end
