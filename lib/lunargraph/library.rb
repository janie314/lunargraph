...
module Lunargraph
  ...
  class Library
    ...
    # @param workspace [Lunargraph::Workspace]
    # @param name [String, nil]
    def initialize workspace = Lunargraph::Workspace.new, name = nil
      @workspace = workspace
      @name = name
      @synchronized = false
    end
    ...
    def create filename, text
      ...
      mutex.synchronize do
        ...
        @synchronized = false
        source = Lunargraph::Source.load_string(text, filename)
    ...
    def create_from_disk *filenames
      ...
      mutex.synchronize do
        sources = filenames
                  ...
                  .map { |filename| Lunargraph::Source.load_string(File.read(filename), filename) }
    ...
    def definitions_at filename, line, column
      ...
      if cursor.comment?
        ...
        offset = Lunargraph::Position.to_offset(source.code, Lunargraph::Position.new(line, column))
    ...
    def references_from filename, line, column, strip: false, only: false
      ...
      files.uniq(&:filename).each do |source|
        ...
        if strip && (match = cursor.word.match(/^[^a-z0-9_]+/i))
          found.map! do |loc|
            Lunargraph::Location.new(loc.filename,
                                     Lunargraph::Range.from_to(loc.range.start.line, loc.range.start.column + match[0].length, loc.range.ending.line,
    ...
    def locate_ref location
      ...
      return if map.nil?
      pin = map.requires.find { |p| p.location.range.contain?(location.range.start) }
      return nil if pin.nil?
      workspace.require_paths.each do |path|
        full = Pathname.new(path).join("#{pin.name}.rb").to_s
        next unless source_map_hash.key?(full)
        return Location.new(full, Lunargraph::Range.from_to(0, 0, 0, 0))
      end
      nil
    ...
    # @param name [String, nil]
    # @return [Lunargraph::Library]
    def self.load directory = '', name = nil
      Lunargraph::Library.new(Lunargraph::Workspace.new(directory), name)
    end
    ...
    def next_map
      ...
      mutex.synchronize do
        ...
        if src
          ...
          source_map_hash[src.filename] = Lunargraph::SourceMap.map(src)
    ...
    def handle_file_not_found filename, error
      ...
      Lunargraph.logger.debug "#{filename} is not cataloged in the ApiMap"
      nil
    end
    ...
    def maybe_map source
      ...
      if source_map_hash.key?(source.filename)
        ...
        if source.synchronized?
          new_map = Lunargraph::SourceMap.map(source)
        ...
        source_map_hash[source.filename] = Lunargraph::SourceMap.map(source)
        find_external_requires(source_map_hash[source.filename])
        @synchronized = false
      end
    end
