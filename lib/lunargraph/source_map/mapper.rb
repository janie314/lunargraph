  ...
  class SourceMap
    ...
    class Mapper
      ...
      def map source
        ...
        [@pins, @locals]
        # rescue Exception => e
        #   Lunargraph.logger.warn "Error mapping #{source.filename}: [#{e.class}] #{e.message}"
        #   Lunargraph.logger.warn e.backtrace.join("\n")
        #   [[], []]
      end
      ...
      # @param position [Lunargraph::Position]
      # @return [Lunargraph::Pin::Closure]
      def closure_at(position)
        pins.reverse.find { |pin| pin.is_a?(Pin::Closure) and pin.location.range.contain?(position) }
      end

      def process_comment source_position, comment_position, comment
        return unless DIRECTIVE_REGEXP.match?(comment.encode('UTF-8', invalid: :replace, replace: '?'))
        cmnt = remove_inline_comment_hashes(comment)
        parse = Lunargraph::Source.parse_docstring(cmnt)
        last_line = 0
        # @param d [YARD::Tags::Directive]
        parse.directives.each do |d|
          line_num = find_directive_line_number(cmnt, d.tag.tag_name, last_line)
          pos = Lunargraph::Position.new(comment_position.line + line_num - 1, comment_position.column)
      ...
      def process_directive source_position, comment_position, directive
        docstring = Lunargraph::Source.parse_docstring(directive.tag.text).to_docstring
        ...
        when 'method'
          ...
          begin
            src = Lunargraph::Source.load_string("def #{directive.tag.name};end", @source.filename)
            ...
            shifted = Lunargraph::Position.new(comment_position.line,
                                               @code.lines[comment_position.line].to_s.chomp.length)
            # @todo: Smelly instance variable access
            gen_pin.instance_variable_set(:@comments, docstring.all.to_s)
            gen_pin.instance_variable_set(:@location, Lunargraph::Location.new(@filename, Range.new(shifted, shifted)))
        ...
        when 'parse'
          begin
            ...
            src = Lunargraph::Source.load_string(directive.tag.text, @source.filename)
      ...
      def process_comment_directives
        return unless DIRECTIVE_REGEXP.match?(@code.encode('UTF-8', invalid: :replace, replace: '?'))
