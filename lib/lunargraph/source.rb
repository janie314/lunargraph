...
module Lunargraph
  ...
  #
  class Source
    autoload :Updater,       'lunargraph/source/updater'
    autoload :Change,        'lunargraph/source/change'
    autoload :Mapper,        'lunargraph/source/mapper'
    autoload :EncodingFixes, 'lunargraph/source/encoding_fixes'
    autoload :Cursor,        'lunargraph/source/cursor'
    autoload :Chain,         'lunargraph/source/chain'
    autoload :SourceChainer, 'lunargraph/source/source_chainer'
    ...
    def initialize code, filename = nil, version = 0
      ...
      @domains = []
      begin
        @node, @comments = Lunargraph::Parser.parse_with_comments(@code, filename)
        @parsed = true
    ...
    def from_to l1, c1, l2, c2
      b = Lunargraph::Position.line_char_to_offset(@code, l1, c1)
      e = Lunargraph::Position.line_char_to_offset(@code, l2, c2)
      @code[b..e - 1]
    end
    ...
    def string_at? position
      if Parser.rubyvm?
        string_ranges.each do |range|
          ...
          elsif last_updater&.changes&.one? && range.contain?(last_updater.changes.first.range.start)
            return true
          end
        end
      else
        ...
        string_nodes.each do |node|
          ...
          if node.type == :dstr
            ...
            inner_code = at(Lunargraph::Range.new(inner_range.start, position))
    ...
    def inner_folding_ranges top, result = [], parent = nil
      ...
      if FOLDING_NODE_TYPES.include?(top.type) && !(%i[ARRAY LIST].include?(top.type) && parent == :HASH)
    ...
    def inner_tree_at node, position, stack
      ...
      return unless here.contain?(position) || colonized(here, position, node)
      stack.unshift node
