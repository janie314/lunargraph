...
describe Lunargraph::Workspace do
  ...
  it "does not merge non-workspace sources" do
    source = Lunargraph::Source.load_string("exit", "not_ruby.txt")
  ...
  it "updates sources" do
    ...
    updated = Lunargraph::Source.load_string('puts "updated"', file_path)
  ...
  it "raises an exception for workspace size limits" do
    config = double(:config, calculated: Array.new(Lunargraph::Workspace::Config::MAX_FILES + 1), max_files: Lunargraph::Workspace::Config::MAX_FILES)

    expect {
      described_class.new(".", config)
    }.to raise_error(Lunargraph::WorkspaceTooLargeError)
  end

  it "allows for unlimited files in config" do
    gemspec_file = File.join(dir_path, "test.gemspec")
    File.write(gemspec_file, "")
    calculated = Array.new(Lunargraph::Workspace::Config::MAX_FILES + 1) { gemspec_file }
