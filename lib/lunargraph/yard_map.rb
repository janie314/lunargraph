...
module Lunargraph
  ...
  class YardMap
    ...
    autoload :Cache,       'lunargraph/yard_map/cache'
    autoload :Mapper,      'lunargraph/yard_map/mapper'
    autoload :RdocToYard,  'lunargraph/yard_map/rdoc_to_yard'
    autoload :Helpers,     'lunargraph/yard_map/helpers'
    autoload :ToMethod,    'lunargraph/yard_map/to_method'
    ...
    def rebindable_method_names
      @rebindable_method_names ||= pins_by_class(Pin::Method)
                                   .select { |pin| pin.comments&.include?('@yieldself') }
                                   .map(&:name)
                                   .push('instance_eval', 'instance_exec', 'class_eval', 'class_exec', 'module_eval', 'module_exec',
                                         'define_method')
                                   .to_set
    end
    ...
    def load_yardoc y
      ...
      Lunargraph::Logging.logger.warn "Error loading yardoc '#{y}' #{e.class} #{e.message}"
      yardocs.delete y
      nil
    end
    ...
    def require_reference path
      ...
      spec.full_require_paths.each do |rp|
        ...
        return Lunargraph::Location.new(file, Lunargraph::Range.from_to(0, 0, 0, 0))
      end
      nil
    ...
    def process_error(req, _result, already_errored, yd = 1)
    ...
    def process_yardoc y, spec = nil
      return [] if y.nil?
      if spec
        cache = Lunargraph::Cache.load('gems', "#{spec.name}-#{spec.version}.ser")
        return cache if cache
      end
      size = Dir.glob(File.join(y, '**', '*')).sum { |f| File.size(f) }
      if !size.nil? && size > 20_000_000
        Lunargraph::Logging.logger.warn "Yardoc at #{y} is too large to process (#{size} bytes)"
        return []
      end
      Lunargraph.logger.info "Loading #{spec.name} #{spec.version} from #{y}"
      load_yardoc y
      result = Mapper.new(YARD::Registry.all, spec).map
      raise NoYardocError, "Yardoc at #{y} is empty" if result.empty?
      Lunargraph::Cache.save 'gems', "#{spec.name}-#{spec.version}.ser", result if spec
      result
    end
    ...
    def spec_for_require path
      ...
      if @gemset[spec.name] && @gemset[spec.name] != spec.version
        begin
          ...
          Lunargraph.logger.warn "Unable to load #{spec.name} #{@gemset[spec.name]} specified by workspace, using #{spec.version} instead"
        end
      end
      spec
    end
