...
module Lunargraph
  class ComplexType
    ...
    module TypeMethods
      ...
      # @return [Boolean]
      def nil_type?
        @nil_type = name.casecmp('nil').zero? if @nil_type.nil?
        @nil_type
      end
      ...
      def namespace
        ...
        @namespace ||= lambda do
          ...
          (name == 'Class' || name == 'Module') && !subtypes.empty? ? subtypes.first.name : name
        end.call
      end
      ...
      def qualify api_map, context = ''
        ...
        if list_parameters?
          Lunargraph::ComplexType.parse("#{fqns}<#{rtypes.map(&:tag).join(', ')}>")
        elsif fixed_parameters?
          Lunargraph::ComplexType.parse("#{fqns}(#{rtypes.map(&:tag).join(', ')})")
        elsif hash_parameters?
          Lunargraph::ComplexType.parse("#{fqns}{#{ltypes.map(&:tag).join(', ')} => #{rtypes.map(&:tag).join(', ')}}")
        else
          Lunargraph::ComplexType.parse(fqns)
        end
      end
