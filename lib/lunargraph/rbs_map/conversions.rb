...
module Lunargraph
  class RbsMap
    # Functions for converting RBS declarations to Lunargraph pins
    #
    module Conversions
      ...
      # @return [void]
      def convert_decl_to_pin decl, _closure
        cursor = pins.length
        case decl
      ...
      def convert_member_to_pin member, closure, context
        ...
        when RBS::AST::Declarations::Base
          ...
          Lunargraph.logger.warn "Skipping member #{member.class}"
        end
        context
      end
      ...
      def class_decl_to_pin decl
        class_pin = Lunargraph::Pin::Namespace.new(
          type: :class,
          name: decl.name.relative!.to_s,
          closure: Lunargraph::Pin::ROOT_PIN,
          comments: decl.comment&.string,
          parameters: decl.type_params.map(&:name).map(&:to_s)
        )
        pins.push class_pin
        if decl.super_class
          pins.push Lunargraph::Pin::Reference::Superclass.new(
            closure: class_pin,
      ...
      def module_decl_to_pin decl
        module_pin = Lunargraph::Pin::Namespace.new(
          type: :module,
          name: decl.name.relative!.to_s,
          closure: Lunargraph::Pin::ROOT_PIN,
          comments: decl.comment&.string
      ...
      def constant_decl_to_pin decl
        ...
        if parts.length > 1
          name = parts.last
          closure = pins.find { |pin| pin && pin.path == parts[0..-2].join('::') }
        else
          name = parts.first
          closure = Lunargraph::Pin::ROOT_PIN
        end
        pin = Lunargraph::Pin::Constant.new(
          name: name,
          closure: closure,
          comments: decl.comment&.string
        )
      ...
      def method_def_to_sigs decl, pin
        decl.types.map do |type|
          parameters, = parts_of_function(type, pin)
          block = (Pin::Signature.new(*parts_of_function(type.block, pin)) if type.block)
      ...
      def parts_of_function type, pin
        ...
        type.type.required_positionals.each do |param|
          ...
          parameters.push Lunargraph::Pin::Parameter.new(decl: :arg, name: name, closure: pin,
                                                         return_type: ComplexType.try_parse(other_type_to_tag(param.type)))
        end
        type.type.optional_positionals.each do |param|
          name = param.name ? param.name.to_s : "arg#{arg_num += 1}"
          parameters.push Lunargraph::Pin::Parameter.new(decl: :optarg, name: name, closure: pin)
        end
        if type.type.rest_positionals
          name = type.type.rest_positionals.name ? type.type.rest_positionals.name.to_s : "arg#{arg_num += 1}"
          parameters.push Lunargraph::Pin::Parameter.new(decl: :restarg, name: name, closure: pin)
        end
        type.type.trailing_positionals.each do |param|
          name = param.name ? param.name.to_s : "arg#{arg_num += 1}"
          parameters.push Lunargraph::Pin::Parameter.new(decl: :arg, name: name, closure: pin)
        end
        type.type.required_keywords.each_key do |orig|
          name = orig ? orig.to_s : "arg#{arg_num += 1}"
          parameters.push Lunargraph::Pin::Parameter.new(decl: :kwarg, name: name, closure: pin)
        end
        type.type.optional_keywords.each_key do |orig|
          name = orig ? orig.to_s : "arg#{arg_num += 1}"
          parameters.push Lunargraph::Pin::Parameter.new(decl: :kwoptarg, name: name, closure: pin)
        end
        if type.type.rest_keywords
          name = type.type.rest_keywords.name ? type.type.rest_keywords.name.to_s : "arg#{arg_num += 1}"
          parameters.push Lunargraph::Pin::Parameter.new(decl: :kwrestarg, name: type.type.rest_keywords.name.to_s,
                                                         closure: pin)
        end
        return_type = ComplexType.try_parse(method_type_to_tag(type))
      ...
      def include_to_pin decl, closure
        pins.push Lunargraph::Pin::Reference::Include.new(
          name: decl.name.relative!.to_s,
          closure: closure
        )
      end
      ...
      def prepend_to_pin decl, closure
        pins.push Lunargraph::Pin::Reference::Prepend.new(
          name: decl.name.relative!.to_s,
          closure: closure
        )
      end
      ...
      def extend_to_pin decl, closure
        pins.push Lunargraph::Pin::Reference::Extend.new(
          name: decl.name.relative!.to_s,
          closure: closure
        )
      end
      ...
      def other_type_to_tag type
        if type.is_a?(RBS::Types::Optional)
          ...
          Lunargraph.logger.warn "Unrecognized RBS type: #{type.class} #{type}"
          'undefined'
        end
      end
    end
