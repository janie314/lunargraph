...
describe Lunargraph::Workspace::Config do
  ...
  before do
    ...
    @orig_env = ENV["LUNARGRAPH_GLOBAL_CONFIG"]
    ENV["LUNARGRAPH_GLOBAL_CONFIG"] = File.join(@global_path, ".lunargraph.yml")
  end

  after do
    ENV["LUNARGRAPH_GLOBAL_CONFIG"] = @orig_env
  ...
  it "reads workspace files from config" do
    ...
    File.open(File.join(workspace_path, ".lunargraph.yml"), "w") do |file|
      file.puts "include:"
      file.puts "  - foo.rb"
      file.puts "exclude:"
      file.puts "  - bar.rb"
  ...
  it "reads workspace files from global config" do
    ...
    File.open(File.join(global_path, ".lunargraph.yml"), "w") do |file|
      file.puts "include:"
      file.puts "  - foo.rb"
      file.puts "exclude:"
      file.puts "  - bar.rb"
