      ...
      module NodeMethods
        ...
        def convert_hash node
          ...
          until index > node.children[0].children.length - 2
            ...
            result[k.children[0]] = Lunargraph::Parser.chain(v)
            index += 2
          end
          result
        end
        ...
        def splatted_node? node
          node?(node.children[0]) &&
            %i[ARRAY LIST].include?(node.children[0].type) &&
        ...
        class << self
          ...
          def synchronized_find_recipient_node cursor
            ...
            tree = source.tree_at(position.line, position.column)
                         .select { |n| %i[FCALL VCALL CALL].include?(n.type) }
            tree.shift while tree.first && !source.code_for(tree.first).strip.end_with?(')') unless source.repaired?
            return tree.first if source.repaired? || source.code[0..offset - 1] =~ /\(\s*$/
            tree.each do |node|
              args = node.children.find { |c| Parser.is_ast_node?(c) && %i[ARRAY ZARRAY LIST].include?(c.type) }
              next unless args
              match = source.code[0..offset - 1].match(/,[^\)]*\z/)
              rng = Lunargraph::Range.from_node(args)
              rng = Lunargraph::Range.new(rng.start, position) if match
          ...
          def maybe_adjust_cursor cursor
            return cursor unless (cursor.source.repaired? && cursor.source.code[cursor.offset - 1] == '(') || [',',
                                                                                                               ' '].include?(cursor.source.code[cursor.offset - 1])
          ...
          def unsynchronized_find_recipient_node cursor
            ...
            position = cursor.position
            offset = cursor.offset
            if /\([A-Zaz0-9_\s]*\z$/.match?(source.code[0..offset - 1])
              tree = source.tree_at(position.line, position.column - 1)
              return tree.first if tree.first && %i[FCALL VCALL CALL].include?(tree.first.type)
              ...
              if match
                ...
                tree.shift while tree.first && !%i[FCALL VCALL CALL].include?(tree.first.type)
                return tree.first if tree.first && %i[FCALL VCALL CALL].include?(tree.first.type)
              end
            end
            nil
        ...
        module DeepInference
          class << self
            ...
            REDUCEABLE = [:BLOCK].freeze
            SKIPPABLE = %i[DEFN DEFS CLASS SCLASS MODULE].freeze
            ...
            def get_return_nodes node
              ...
              if REDUCEABLE.include?(node.type)
                ...
                node.children[1..].each do |cc|
                  result.concat reduce_to_value_nodes(cc.children[1..])
                end
              else
                result.push node
              end
