...
module Lunargraph
  ...
  class ComplexType
    ...
    autoload :TypeMethods, 'lunargraph/complex_type/type_methods'
    autoload :UniqueType,  'lunargraph/complex_type/unique_type'
    ...
    def qualify api_map, context = ''
      types = @items.map do |t|
        next t if %w[Boolean nil void undefined].include?(t.name)
    ...
    def to_rbs
      ((@items.length > 1 ? '(' : '') + @items.map do |item|
        "#{item.namespace}#{item.parameters? ? "[#{item.subtypes.map(&:to_rbs).join(', ')}]" : ''}"
    ...
    class << self
        ...
        strings.each do |type_string|
          ...
          type_string&.each_char do |char|
            if char == '='
              ...
              paren_stack -= 1
              subtype_string += char if paren_stack.zero?
      ...
      def try_parse *strings
        ...
        Lunargraph.logger.info "Error parsing complex type: #{e.message}"
