      ...
      class NodeChainer
        ...
        def generate_links n
          ...
          if n.type == :ITER
            ...
            result.push Chain::Call.new(n.children[-2].to_s, node_to_argchains(n.children.last),
                                        @in_block.positive? || block_passed?(n))
          elsif n.type == :QCALL
            n.children[0..-3].each do |c|
              result.concat generate_links(c)
            end
            result.push Chain::QCall.new(n.children[-2].to_s, node_to_argchains(n.children.last),
                                         @in_block.positive? || block_passed?(n))
          elsif n.type == :ATTRASGN
            result.concat generate_links(n.children[0])
            result.push Chain::Call.new(n.children[1].to_s, node_to_argchains(n.children[2]),
                                        @in_block.positive? || block_passed?(n))
          elsif n.type == :VCALL
            result.push Chain::Call.new(n.children[0].to_s, [], @in_block.positive? || block_passed?(n))
          elsif n.type == :FCALL
            result.push Chain::Call.new(n.children[0].to_s, node_to_argchains(n.children[1]),
                                        @in_block.positive? || block_passed?(n))
          elsif n.type == :SELF
            result.push Chain::Head.new('self')
          elsif n.type == :ZSUPER
            result.push Chain::ZSuper.new('super', @in_block.positive? || block_passed?(n))
          elsif n.type == :SUPER
            result.push Chain::Call.new('super', node_to_argchains(n.children.last),
                                        @in_block.positive? || block_passed?(n))
          elsif %i[COLON2 COLON3 CONST].include?(n.type)
            const = unpack_name(n)
            result.push Chain::Constant.new(const)
          elsif %i[LVAR LASGN DVAR].include?(n.type)
            result.push Chain::Call.new(n.children[0].to_s)
          elsif %i[IVAR IASGN].include?(n.type)
            result.push Chain::InstanceVariable.new(n.children[0].to_s)
          elsif %i[CVAR CVASGN].include?(n.type)
            result.push Chain::ClassVariable.new(n.children[0].to_s)
          elsif %i[GVAR GASGN].include?(n.type)
            ...
            result.push Chain::Or.new([NodeChainer.chain(n.children[0], @filename),
                                       NodeChainer.chain(n.children[1], @filename)])
          elsif n.type == :begin
            result.concat generate_links(n.children[0])
          elsif n.type == :BLOCK_PASS
            result.push Chain::BlockVariable.new("&#{n.children[1].children[0]}")
          elsif n.type == :HASH
        ...
        def node_to_argchains node
          ...
          if %i[ZARRAY ARRAY LIST].include?(node.type)
            ...
            result = node_to_argchains(node.children[0])
            result.push Chain.new([Chain::BlockVariable.new("&#{node.children[1].children[0]}")])
            result
          else
            []
          end
