...
describe Lunargraph::YardMap do
  it "removes requires on change" do
    yard_map = described_class.new(required: ["set"])
  ...
  it "collects pins from gems" do
    # Assuming the parser gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["parser"])
  ...
  it "tracks unresolved requires" do
    yard_map = described_class.new(required: ["not_valid"])
  ...
  it "tracks missing documentation" do
    ...
    #   without a yardoc.
    yard_map = described_class.new(required: ["set", "not_valid"])
  ...
  it "ignores duplicate requires" do
    # Assuming the parser gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["parser", "parser"])
  ...
  it "ignores multiple paths to the same gem" do
    # Assuming the parser gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["parser", "parser/ast"])
    pins = yard_map.pins.select { |p| p.path == "Parser::AST::Node#location" }
    expect(pins.length).to eq(1)
  end

  it "adds superclass references" do
    # Asssuming the yard gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["yard"])
  ...
  it "adds include references" do
    # Asssuming the ast gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["ast"])
    api_map = Lunargraph::ApiMap.new
    api_map.index yard_map.pins
  ...
  it "includes gem dependencies based on attribute" do
    # Assuming the parser gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["parser"])
    expect(yard_map.with_dependencies?).to be(true)
    parser = yard_map.path_pin("Parser")
    expect(parser).to be
  ...
  it "excludes gem dependencies based on attribute" do
    # Assuming the parser gem exists because it's a Lunargraph dependency
    yard_map = described_class.new(required: ["parser"], with_dependencies: false)
    expect(yard_map.with_dependencies?).to be(false)
    parser = yard_map.path_pin("Parser")
    expect(parser).to be
    ast = yard_map.path_pin("AST")
  ...
  it "finds require paths in gems" do
    ...
    expect(location).to be_a(Lunargraph::Location)
  end

  it "returns nil for require paths without gems" do
    yard_map = described_class.new
