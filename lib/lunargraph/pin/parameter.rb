        ...
        if closure.is_a?(Pin::Block) && closure.receiver
          ...
          meths.each do |meth|
            if meth.docstring.has_tag?(:yieldparam_single_parameter)
              ...
              unless yps[index].nil? || yps[index].types.nil? || yps[index].types.empty?
                return ComplexType.try_parse(yps[index].types.first).self_to(chain.base.infer(api_map, closure, locals).namespace).qualify(
                  api_map, meth.context.namespace
                )
              end
            end
          end
