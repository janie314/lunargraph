...
module Lunargraph
  class Workspace
    ...
    class Config
      ...
      # The workspace's .lunargraph.yml can override this value.
      MAX_FILES = 5000
      ...
      def calculated
        unless @calculated || directory.empty? || directory == '*'
          Lunargraph.logger.info "Indexing workspace files in #{directory}"
        end
      ...
      def global_config_path
        ENV['LUNARGRAPH_GLOBAL_CONFIG'] ||
          File.join(Dir.home, '.config', 'lunargraph', 'config.yml')
      end

      # @return [String]
      def workspace_config_path
        return '' if @directory.empty?
        File.join(@directory, '.lunargraph.yml')
      end
