...
require 'lunargraph/parser/rubyvm/node_wrapper'
  ...
  module Parser
    module Rubyvm
      module ClassMethods
        ...
        # @sg-ignore
        def parse_with_comments code, _filename = nil
        ...
        # @sg-ignore
        def parse code, _filename = nil, _line = 0
          node = RubyVM::AbstractSyntaxTree.parse(code).children[2]
        ...
        def inner_node_references name, top
          ...
          elsif Parser.is_ast_node?(top) && top.to_s.include?(":#{name}")
        ...
        def process_node *args
          Lunargraph::Parser::NodeProcessor.process(*args)
        end
        ...
        def recipient_node tree
          tree.each_with_index do |node, idx|
            return tree[idx + 1] if %i[ARRAY ZARRAY
                                       LIST].include?(node.type) && tree[idx + 1] && %i[FCALL VCALL
                                                                                        CALL].include?(tree[idx + 1].type)
          end
          nil
        end
