  ...
  class Source
    ...
    class SourceChainer
      ...
      def chain
        ...
        return Chain.new([Chain::Literal.new('Integer'), Chain::UNDEFINED_CALL]) if /^[0-9]+\.$/.match?(phrase)
        ...
        chain = Parser.chain(node, source.filename, parent && %i[ITER block].include?(parent.type))
        if source.repaired? || !source.parsed? || !source.synchronized?
