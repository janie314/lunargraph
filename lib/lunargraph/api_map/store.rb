...
module Lunargraph
  class ApiMap
    class Store
      # @return [Enumerable<Lunargraph::Pin::Base>]
      attr_reader :pins

      # @param pins [Enumerable<Lunargraph::Pin::Base>]
      def initialize pins = []
        @pins = pins
        index
      end
      ...
      # @return [Enumerable<Lunargraph::Pin::Base>]
      def namespace_pins
        pins_by_class(Lunargraph::Pin::Namespace)
      end

      # @return [Enumerable<Lunargraph::Pin::Method>]
      def method_pins
        pins_by_class(Lunargraph::Pin::Method)
      end
      ...
      def pins_by_class klass
        @pin_select_cache[klass] ||= @pin_class_hash.each_with_object(Set.new) do |(key, o), n|
          n.merge(o) if key <= klass
        end
      end
      ...
      # @param fqns [String]
      # @return [Array<Lunargraph::Pin::Namespace>]
      def fqns_pins fqns
        return [] if fqns.nil?
        if fqns.include?('::')
          parts = fqns.split('::')
      ...
      # @return [Enumerable<Lunargraph::Pin::Symbol>]
      def symbols
      ...
      def index
        ...
        pins_by_class(Pin::Reference::Override).each do |ovr|
          ...
          next if pin.nil?
          new_pin = (get_path_pins(pin.path.sub('#initialize', '.new')).first if pin.path.end_with?('#initialize'))
          (ovr.tags.map(&:tag_name) + ovr.delete).uniq.each do |tag|
            pin.docstring.delete_tags tag.to_sym
            new_pin&.docstring&.delete_tags tag.to_sym
          end
