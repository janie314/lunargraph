...
module Lunargraph
  module Pin
    ...
    class Method < Closure
      include Lunargraph::Parser::NodeMethods
      ...
      def initialize visibility: :public, explicit: true, parameters: [], node: nil, attribute: false, signatures: nil,
                     anon_splat: false, **splat
        super(**splat)
      ...
      def completion_item_kind
        attribute? ? Lunargraph::LanguageServer::CompletionItemKinds::PROPERTY : Lunargraph::LanguageServer::CompletionItemKinds::METHOD
      end

      def symbol_kind
        attribute? ? Lunargraph::LanguageServer::SymbolKinds::PROPERTY : LanguageServer::SymbolKinds::METHOD
      end
      ...
      def param_type_from_name(tag, name)
        param = tag.tags(:param).find { |t| t.name == name }
      ...
      def infer_from_return_nodes api_map
        ...
        returns_from(method_body_node).each do |n|
          ...
          chain = Lunargraph::Parser.chain(n, location.filename)
