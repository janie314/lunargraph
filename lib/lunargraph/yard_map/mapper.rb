...
module Lunargraph
  class YardMap
    class Mapper
      autoload :ToMethod, 'lunargraph/yard_map/mapper/to_method'
      autoload :ToNamespace, 'lunargraph/yard_map/mapper/to_namespace'
      autoload :ToConstant, 'lunargraph/yard_map/mapper/to_constant'
      ...
      def generate_pins code_object
        ...
        when YARD::CodeObjects::NamespaceObject
          ...
          if code_object.is_a?(YARD::CodeObjects::ClassObject) && !code_object.superclass.nil?
            ...
            result.push Lunargraph::Pin::Reference::Superclass.new(name: superclass, closure: nspin)
          end
          code_object.class_mixins.each do |m|
            result.push Lunargraph::Pin::Reference::Extend.new(closure: nspin, name: m.path)
          end
          code_object.instance_mixins.each do |m|
            result.push Lunargraph::Pin::Reference::Include.new(
