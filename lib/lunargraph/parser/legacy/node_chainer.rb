        ...
        def generate_links n
          ...
          if n.type == :block
            ...
            if n.children[0].is_a?(::Parser::AST::Node)
              result.concat generate_links(n.children[0])
              args = n.children[2..].map do |c|
                NodeChainer.chain(c)
              end
              result.push Chain::Call.new(n.children[1].to_s, args, @in_block.positive? || block_passed?(n))
            ...
            if n.children[0].is_a?(::Parser::AST::Node)
              ...
              result.push Chain::QCall.new(n.children[1].to_s, args, @in_block.positive? || block_passed?(n))
            elsif n.children[0].nil?
              args = n.children[2..].map do |c|
                NodeChainer.chain(c)
              end
            ...
            result.push Chain::Call.new('super', args, @in_block.positive? || block_passed?(n))
          elsif n.type == :const
            const = unpack_name(n)
            ...
            result.push Chain::BlockVariable.new("&#{n.children[0].children[0]}")
          elsif n.type == :hash
