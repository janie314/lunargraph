...
module Lunargraph
  ...
  #
  class TypeChecker
    autoload :Problem,  'lunargraph/type_checker/problem'
    autoload :ParamDef, 'lunargraph/type_checker/param_def'
    autoload :Rules,    'lunargraph/type_checker/rules'
    autoload :Checks,   'lunargraph/type_checker/checks'
    ...
    def initialize filename, api_map: nil, level: :normal
      ...
      @api_map = api_map || Lunargraph::ApiMap.load(File.dirname(filename))
    ...
    class << self
      ...
      def load filename, level = :normal
        source = Lunargraph::Source.load(filename)
      ...
      def load_string code, filename = nil, level = :normal
        source = Lunargraph::Source.load_string(code, filename)
        api_map = Lunargraph::ApiMap.new
        api_map.map(source)
        new(filename, api_map: api_map, level: level)
      end
      ...
      if declared.undefined?
        ...
        unless pin.node.nil? || declared.void? || virtual_pin?(pin) || abstract?(pin)
          ...
          if inferred.undefined?
            ...
            unless if rules.rank > 1
                     all_types_match?(api_map, inferred,
                                      declared)
                   else
                     any_types_match?(api_map, declared, inferred)
                   end
              result.push Problem.new(pin.location,
                                      "Declared return type #{declared} does not match inferred type #{inferred} for #{pin.path}", pin: pin)
            end
          end
        end
      end
    ...
    def const_problems
      ...
      result = []
      Lunargraph::Parser::NodeMethods.const_nodes_from(source_map.source.node).each do |const|
        rng = Lunargraph::Range.from_node(const)
        chain = Lunargraph::Parser.chain(const, filename)
        ...
        if pins.empty?
          result.push Problem.new(location, "Unresolved constant #{Lunargraph::Parser::NodeMethods.unpack_name(const)}")
    ...
    def call_problems
      result = []
      Lunargraph::Parser::NodeMethods.call_nodes_from(source_map.source.node).each do |call|
        rng = Lunargraph::Range.from_node(call)
        next if @marked_ranges.any? { |d| d.contain?(rng.start) }
        chain = Lunargraph::Parser.chain(call, filename)
        block_pin = source_map.locate_block_pin(rng.start.line, rng.start.column)
        location = Location.new(filename, rng)
        locals = source_map.locals_at(location)
        ...
        if type.undefined? && !rules.ignore_all_undefined?
          ...
          if (!found || found.is_a?(Pin::BaseVariable) || (closest.defined? && internal_or_core?(found))) && !(closest.parameterized? || ignored_pins.include?(found))
    ...
    def argument_problems_for chain, api_map, block_pin, locals, location
      ...
      until base.links.length == 1 && base.undefined?
        ...
        if pins.first.is_a?(Pin::Method)
          # @type [Pin::Method]
          pin = pins.first
          ap = if base.links.last.is_a?(Lunargraph::Source::Chain::ZSuper)
    ...
    def kwarg_problems_for argchain, api_map, block_pin, locals, location, pin, params, first
      ...
      pin.signatures.first.parameters[first..].each_with_index do |par, cur|
        idx = first + cur
    ...
    def param_hash(pin)
      ...
      tags.each do |tag|
        ...
        result[tag.name.to_s] = {
          ...
          qualified: Lunargraph::ComplexType.try_parse(*tag.types).qualify(api_map, pin.full_context.namespace)
        }
      end
      result
    end
    ...
    def declared_externally? pin
      ...
      chain = Lunargraph::Parser.chain(pin.assignment, filename)
      rng = Lunargraph::Range.from_node(pin.assignment)
      block_pin = source_map.locate_block_pin(rng.start.line, rng.start.column)
    ...
    def parameterized_arity_problems_for(pin, parameters, arguments, location)
            ...
            if kwargs.empty?
              ...
              parameters.each do |param|
                ...
                if kwargs.key?(param.name.to_sym)
                  ...
                  if arguments.last.links.last.is_a?(Lunargraph::Source::Chain::Hash) && arguments.last.links.last.splatted?
                    return []
                  end
      ...
      if req + add_params < unchecked.length
        ...
        return [] if unchecked.length == req + opt + 1 && unchecked.last.links.last.is_a?(Source::Chain::BlockVariable)
        if req + add_params + 1 == unchecked.length && any_splatted_call?(unchecked.map(&:node)) && (parameters.map(&:decl) & %i[
          kwarg kwoptarg kwrestarg
        ]).any?
          return []
        end
        return [] if arguments.length - req == parameters.count { |p| %i[optarg kwoptarg].include?(p.decl) }
        return [Problem.new(location, "Too many arguments to #{pin.path}")]
      elsif unchecked.length < req - settled_kwargs && (arguments.empty? || (!arguments.last.splat? && !arguments.last.links.last.is_a?(Lunargraph::Source::Chain::Hash)))
        # HACK: Kernel#raise signature is incorrect in Ruby 2.7 core docs.
        # See https://github.com/castwide/lunargraph/issues/418
    ...
    def optional_param_count(parameters)
      parameters.count { |p| p.decl == :optarg }
    end
    ...
    def fake_args_for(pin)
      ...
      pin.parameters.each do |pin|
        if %i[kwarg kwoptarg kwrestarg].include?(pin.decl)
          ...
          args.push Lunargraph::Source::Chain.new([Lunargraph::Source::Chain::Variable.new(pin.name)], nil, true)
        else
          args.push Lunargraph::Source::Chain.new([Lunargraph::Source::Chain::Variable.new(pin.name)])
        end
      end
      args.push Lunargraph::Parser.chain_string('{}') if with_opts
      args.push Lunargraph::Parser.chain_string('&') if with_block
      args
    end
