...
module Lunargraph
  ...
  #
  class Workspace
    autoload :Config, 'lunargraph/workspace/config'
    ...
    # @return [Lunargraph::Workspace::Config]
    def config
      @config ||= Lunargraph::Workspace::Config.new(directory)
    end
    ...
    # @return [Array<Lunargraph::Source>]
    def sources
      source_hash.values
    end
    ...
    # @return [Hash{String => Lunargraph::Source}]
    def source_hash
      @source_hash ||= {}
    end
    ...
    def load_sources
      ...
      config.calculated.each do |filename|
        source_hash[filename] = Lunargraph::Source.load(filename)
      rescue Errno::ENOENT => e
        Lunargraph.logger.warn("Error loading #{filename}: [#{e.class}] #{e.message}")
      end
    end
      ...
      gemspecs.each do |file|
        ...
        if s.success?
          begin
            ...
            Lunargraph.logger.warn "Error reading #{file}: [#{e.class}] #{e.message}"
          end
        else
          Lunargraph.logger.warn "Error reading #{file}"
    ...
    def require_plugins
      config.plugins.each do |plugin|
        ...
        Lunargraph.logger.warn "Failed to load plugin '#{plugin}'"
      end
    end
  end
end
