...
module Lunargraph
  module LanguageServer
    ...
    #
    class Host
      autoload :Diagnoser,     'lunargraph/language_server/host/diagnoser'
      autoload :Cataloger,     'lunargraph/language_server/host/cataloger'
      autoload :Sources,       'lunargraph/language_server/host/sources'
      autoload :Dispatch,      'lunargraph/language_server/host/dispatch'
      autoload :MessageWorker, 'lunargraph/language_server/host/message_worker'
      ...
      # @return [Lunargraph::LanguageServer::Message::Base] The message handler.
      def receive request
        if request['method']
          ...
          begin
            ...
            message.set_error Lunargraph::LanguageServer::ErrorCodes::INTERNAL_ERROR, "[#{e.class}] #{e.message}"
          end
          message
        elsif request['id']
      ...
      # Lunargraph::Library#create_from_disk.
      #
      ...
      def prepare directory, name = nil
        ...
        begin
          lib = Lunargraph::Library.load(path, name)
          libraries.push lib
          async_library_map lib
        rescue WorkspaceTooLargeError => e
          send_notification 'window/showMessage', {
            'type' => Lunargraph::LanguageServer::MessageTypes::WARNING,
            'message' => e.message
          }
        end
      end
      ...
      def locate_pins params
        ...
        result.concat library.path_pins(params['data']['path']) if params['data']['path']
      ...
      # @return [Lunargraph::SourceMap::Completion]
      ...
      def generate_updater params
        ...
        params['contentChanges'].each do |recvd|
          ...
          changes.push Lunargraph::Source::Change.new(
            (if chng['range'].nil?
               nil
             else
               Lunargraph::Range.from_to(chng['range']['start']['line'], chng['range']['start']['character'],
      ...
      def check_diff uri, change
        ...
        position = Lunargraph::Position.from_offset(source.code, diff.position)
        {
          'range' => {
            'start' => {
              'line' => position.line,
      ...
      rescue Lunargraph::FileNotFoundError
        change
      end
      ...
      def do_async_library_map library, uuid = nil
        ...
        while library.next_map
          next unless uuid
          cur = ((library.source_map_hash.keys.length.to_f / total) * 100).to_i
          next unless cur > pct && (cur % mod).zero?
          pct = cur
          send_notification '$/progress', {
            token: uuid,
            value: {
