...
module Lunargraph
  module Pin
    ...
    class Base
      ...
      # @param location [Lunargraph::Location, nil]
      # @param kind [Integer]
      # @param closure [Lunargraph::Pin::Closure, nil]
      # @param name [String]
      ...
      #
      # @param other [Lunargraph::Pin::Base, Object]
      # @return [Boolean]
      def nearly? other
      ...
      def docstring
        ...
        @docstring ||= Lunargraph::Source.parse_docstring('').to_docstring
      end
      ...
      # @return [ComplexType]
      def infer api_map
        Lunargraph::Logging.logger.warn 'WARNING: Pin #infer methods are deprecated. Use #typify or #probe instead.'
        type = typify(api_map)
      ...
      def inspect
        "#<#{self.class} `#{path}` at #{location.inspect}>"
      end
      ...
      def parse_comments
        ...
        if comments.nil? || comments.empty? || comments.strip.end_with?('@overload')
          ...
          parse = Lunargraph::Source.parse_docstring(comments)
