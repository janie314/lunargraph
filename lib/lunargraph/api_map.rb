...
module Lunargraph
  ...
  #
  class ApiMap
    autoload :Cache,          'lunargraph/api_map/cache'
    autoload :SourceToYard,   'lunargraph/api_map/source_to_yard'
    autoload :Store,          'lunargraph/api_map/store'
    autoload :BundlerMethods, 'lunargraph/api_map/bundler_methods'
    ...
    def self.load directory
      api_map = new
      workspace = Lunargraph::Workspace.new(directory)
    ...
    def rebindable_method_names
      @rebindable_method_names ||= begin
        ...
        result = %w[instance_eval instance_exec class_eval class_exec module_eval module_exec
                    define_method].to_set
    ...
    #
    # @return [Enumerable<Lunargraph::Pin::Keyword>]
    def keyword_pins
    ...
    # @return [Array<Lunargraph::Pin::InstanceVariable>]
    ...
    #
    # @param namespace [String] A fully qualified namespace
    # @return [Array<Lunargraph::Pin::ClassVariable>]
    ...
    # @return [Array<Lunargraph::Pin::GlobalVariable>]
    ...
    # @return [Array<Lunargraph::Pin::Method>]
    ...
    #
    # @param complex_type [Lunargraph::ComplexType] The complex type of the namespace
    ...
    def get_method_stack fqns, name, scope: :instance
      get_methods(fqns, scope: scope, visibility: %i[private protected public]).select { |p| p.name == name }
    end
    ...
    # @return [Lunargraph::ApiMap::Cache]
    attr_reader :cache
    ...
    def inner_get_methods fqns, scope, visibility, deep, skip, no_core = false
      ...
      if deep && scope == :instance
        store.get_prepends(fqns).reverse_each do |im|
          fqim = qualify(im, fqns)
          result.concat inner_get_methods(fqim, scope, visibility, deep, skip, true) unless fqim.nil?
        end
      end
      result.concat(store.get_methods(fqns, scope: scope, visibility: visibility).sort_by(&:name))
      if deep
        if scope == :instance
          store.get_includes(fqns).reverse_each do |im|
            fqim = qualify(im, fqns)
            result.concat inner_get_methods(fqim, scope, visibility, deep, skip, true) unless fqim.nil?
          end
          fqsc = qualify_superclass(fqns)
          result.concat inner_get_methods(fqsc, scope, visibility, true, skip, no_core) if !fqsc.nil? && !fqsc.nil?
        else
          store.get_extends(fqns).reverse_each do |em|
            fqem = qualify(em, fqns)
            result.concat inner_get_methods(fqem, :instance, visibility, deep, skip, true) unless fqem.nil?
          end
          fqsc = qualify_superclass(fqns)
          result.concat inner_get_methods(fqsc, scope, visibility, true, skip, true) if !fqsc.nil? && !fqsc.nil?
    ...
    def get_namespace_type fqns
      ...
      pin = store.get_path_pins(fqns).find { |p| p.is_a?(Pin::Namespace) }
