  ...
  module AbstractSyntaxTree
    ...
    class NodeWrapper < SimpleDelegator
      ...
      def self.from(node, code)
        return node unless node.is_a?(RubyVM::AbstractSyntaxTree::Node) && !node.is_a?(SimpleDelegator)
      ...
      def children
        @children ||= __getobj__.children.map do |node|
          NodeWrapper.from(node, @code)
        end
      end
    end
  end
