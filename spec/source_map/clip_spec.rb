describe Lunargraph::SourceMap::Clip do
  let(:api_map) { Lunargraph::ApiMap.new }

  it "completes constants" do
    orig = Lunargraph::Source.load_string("File")
    updater = Lunargraph::Source::Updater.new(nil, 1, [
      Lunargraph::Source::Change.new(Lunargraph::Range.from_to(0, 4, 0, 4), "::")
    ])
    source = orig.synchronize(updater)
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(0, 6))
  ...
  it "completes class variables" do
    source = Lunargraph::Source.load_string("@@foo = 1; @@f", "test.rb")
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(0, 13))
    clip = described_class.new(api_map, cursor)
    comp = clip.complete
  ...
  it "completes instance variables" do
    source = Lunargraph::Source.load_string("@foo = 1; @f", "test.rb")
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(0, 11))
    comp = clip.complete
    expect(comp.pins.map(&:name)).to include("@foo")
  end
  ...
  it "completes symbols" do
    source = Lunargraph::Source.load_string("$foo = :foo; :f", "test.rb")
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(0, 15))
    comp = clip.complete
    expect(comp.pins.map(&:name)).to include(":foo")
  end
  ...
  it "defines core constants" do
    source = Lunargraph::Source.load_string("String")
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(0, 0))
    clip = described_class.new(api_map, cursor)
    pins = clip.define
  ...
  it "signifies core methods" do
    source = Lunargraph::Source.load_string("File.dirname()")
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(0, 13))
    clip = described_class.new(api_map, cursor)
    pins = clip.signify
  ...
  it "detects local variables" do
    ...
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(2, 0))
    clip = described_class.new(api_map, cursor)
  ...
  it "detects local variables passed into blocks" do
    ...
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(4, 0))
    clip = described_class.new(api_map, cursor)
    expect(clip.locals.map(&:name)).to include("x")
  end
  ...
  it "ignores local variables assigned after blocks" do
    ...
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(3, 0))
    clip = described_class.new(api_map, cursor)
  ...
  it "puts local variables first in completion results" do
    ...
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(4, 7))
    clip = described_class.new(api_map, cursor)
    pins = clip.complete.pins
    expect(pins.first).to be_a(Lunargraph::Pin::LocalVariable)
  ...
  it "completes constants only for leading double colons" do
    ...
    api_map.map source
    cursor = source.cursor_at(Lunargraph::Position.new(1, 8))
    clip = described_class.new(api_map, cursor)
    pins = clip.complete.pins
    expect(pins.all? { |p| p.is_a?(Lunargraph::Pin::Namespace) || p.is_a?(Lunargraph::Pin::Constant) }).to be(true)
  end
  ...
  it "completes partially completed constants" do
    ...
    api_map = Lunargraph::ApiMap.new
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(2, 7))
    pins = clip.complete.pins
  ...
  it "completes partially completed inner constants" do
    ...
    api_map = Lunargraph::ApiMap.new
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(4, 12))
    pins = clip.complete.pins
    expect(pins.length).to eq(1)
  ...
  it "completes unstarted inner constants" do
    ...
    api_map = Lunargraph::ApiMap.new
    api_map.map source
    cursor = api_map.clip_at("test.rb", Lunargraph::Position.new(4, 11))
  ...
  it "does not define arbitrary comments" do
    ...
    api_map = Lunargraph::ApiMap.new
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(3, 10))
  ...
  it "infers types from named macros" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(8, 14))
  ...
  it "infers method types from return nodes" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(4, 6))
    type = clip.infer
  ...
  it "infers multiple method types from return nodes" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(8, 6))
    type = clip.infer
  ...
  it "infers return types from method calls" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(6, 6))
    type = clip.infer
  ...
  it "infers complex return types from method calls" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(3, 7))
    type = clip.infer
  ...
  it "infers return types from instance variables" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(7, 6))
    type = clip.infer
    expect(type.tag).to eq("Hash")
  end
  ...
  it "infers implicit return types from singleton methods" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(6, 10))
    type = clip.infer
    expect(type.tag).to eq("String")
  end
  ...
  it "infers nil for empty methods" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(2, 6))
    type = clip.infer
  ...
  it "handles missing type annotations in @type tags" do
    ...
    api_map = Lunargraph::ApiMap.new
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(3, 7))
    expect {
  ...
  it "completes unfinished constant chains with trailing nodes" do
    ...
    updater = Lunargraph::Source::Updater.new("test.rb", 1, [
      Lunargraph::Source::Change.new(Lunargraph::Range.from_to(6, 14, 6, 14), "::")
    ])
    source = orig.synchronize(updater)
    api_map.map source
    clip = api_map.clip_at("test.rb", Lunargraph::Position.new(6, 16))
  ...
  it "resolves conflicts between namespaces names" do
    ...
    clip1a = api_map.clip_at("test.rb", Lunargraph::Position.new(12, 9))
    expect(clip1a.infer.to_s).to eq("Other::Foo")
    clip1b = api_map.clip_at("test.rb", Lunargraph::Position.new(12, 13))
    ...
    clip2a = api_map.clip_at("test.rb", Lunargraph::Position.new(15, 9))
    # expect(clip2a.infer.rooted?).to be(true)
    expect(clip2a.infer.to_s).to eq("Foo")
    clip2b = api_map.clip_at("test.rb", Lunargraph::Position.new(15, 13))
  ...
  it "avoids completion inside strings for unsynchronized sources" do
    ...
    updater = Lunargraph::Source::Updater.new(
      ...
      1,
      [
        Lunargraph::Source::Change.new(Lunargraph::Range.from_to(1, 6, 1, 6), ".")
      ]
    )
    updated = source.start_synchronize(updater)
    cursor = updated.cursor_at(Lunargraph::Position.new(1, 7))
  ...
  it "infers deep variables" do
    ...
    code += "v100"
    source = Lunargraph::Source.load_string(code, "test.rb")
    api_map = Lunargraph::ApiMap.new
    api_map.map source
  ...
  it "infers Array#[] types from overloads" do
    ...
    map = Lunargraph::ApiMap.new
    map.map source
    clip = map.clip_at("test.rb", Lunargraph::Position.new(3, 15))
    expect(clip.infer.to_s).to eq("Array<String>, nil")
    clip = map.clip_at("test.rb", Lunargraph::Position.new(4, 15))
    expect(clip.infer.to_s).to eq("Array<String>, nil")
    clip = map.clip_at("test.rb", Lunargraph::Position.new(5, 15))
  ...
  it "detects sibling constants in open scope gates" do
    ...
    pin = clip.complete.pins.find { |p| p.name == "new" }
  ...
  it "completes clips from repaired sources ending with a period" do
    ...
    updater = Lunargraph::Source::Updater.new("test.rb", 1, [
      Lunargraph::Source::Change.new(
        Lunargraph::Range.from_to(1, 31, 1, 31),
        "."
      )
    ])
    updated = source.start_synchronize(updater)
  ...
  it "signifies unsynchronized sources updated with commas" do
    ...
    updater = Lunargraph::Source::Updater.new(
      ...
      2,
      [
        Lunargraph::Source::Change.new(Lunargraph::Range.from_to(7, 32, 7, 32), ",")
      ]
    )
    updated = source.start_synchronize(updater)
    api_map = Lunargraph::ApiMap.new
  ...
  it "signifies empty parentheses" do
    src = Lunargraph::Source.load_string %(
      class Foo
        def bar baz, key: ''
        end
      end
  ...
  it "signifies unsynchronized sources updated with parentheses" do
    ...
    updater = Lunargraph::Source::Updater.new(
      ...
      2,
      [
        Lunargraph::Source::Change.new(Lunargraph::Range.from_to(7, 29, 7, 29), "()")
      ]
    )
    updated = source.start_synchronize(updater)
    api_map = Lunargraph::ApiMap.new
  ...
  it "signifies unsynchronized sources with nested symbols" do
    ...
    updater = Lunargraph::Source::Updater.new(
      ...
      2,
      [
        Lunargraph::Source::Change.new(Lunargraph::Range.from_to(7, 30, 7, 30), "F")
      ]
    )
    updated = source.start_synchronize(updater)
    api_map = Lunargraph::ApiMap.new
