...
module Lunargraph
  ...
  #
  class SourceMap
    autoload :Mapper,        'lunargraph/source_map/mapper'
    autoload :Clip,          'lunargraph/source_map/clip'
    autoload :Completion,    'lunargraph/source_map/completion'
    ...
    def document_symbols
      @document_symbols ||= pins.select do |pin|
    ...
    def try_merge! other_map
      if pins.length != other_map.pins.length || locals.length != other_map.locals.length || requires.map(&:name).uniq.sort != other_map.requires.map(&:name).uniq.sort
        return false
      end
