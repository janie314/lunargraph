...
describe Lunargraph::Library do
  ...
  it "returns a Completion" do
    ...
    library.attach Lunargraph::Source.load_string(%(
      x = 1
      x
    ), "file.rb", 0)
    completion = library.completions_at("file.rb", 2, 7)
    expect(completion).to be_a(Lunargraph::SourceMap::Completion)
  ...
  it "collects references to an instance method symbol" do
    workspace = Lunargraph::Workspace.new("*")
    library = described_class.new(workspace)
    src1 = Lunargraph::Source.load_string(%(
      class Foo
        def bar
        end
      end
    ...
    library.merge src1
    src2 = Lunargraph::Source.load_string(%(
      foo = Foo.new
      foo.bar
      class Other
        def bar; end
  ...
  it "collects stripped references to constant symbols" do
    ...
    locs.each do |l|
      ...
      o1 = Lunargraph::Position.to_offset(code, l.range.start)
      o2 = Lunargraph::Position.to_offset(code, l.range.ending)
  ...
  it "rejects new references from different classes" do
    ...
    expect(foo_new_locs).to eq([Lunargraph::Location.new("test.rb", Lunargraph::Range.from_to(5, 10, 5, 13))])
    obj_new_locs = library.references_from("test.rb", 6, 12)
    expect(obj_new_locs).to eq([Lunargraph::Location.new("test.rb", Lunargraph::Range.from_to(6, 12, 6, 15))])
  end
  ...
  it "synchronizes sources from updaters" do
    ...
    updater = Lunargraph::Source::Updater.new(
      "test.rb",
      2,
      [Lunargraph::Source::Change.new(nil, repl)]
    )
  ...
  it "finds unique references" do
    library = described_class.new(Lunargraph::Workspace.new("*"))
    src1 = Lunargraph::Source.load_string(%(
      class Foo
      end
    ), "src1.rb", 1)
  ...
  it "marks aliases as methods or attributes in completion items" do
    ...
    expect(foo_alias.completion_item_kind).to eq(Lunargraph::LanguageServer::CompletionItemKinds::PROPERTY)
    bar_alias = library.completions_at("test.rb", 10, 14).pins.first
    expect(bar_alias.completion_item_kind).to eq(Lunargraph::LanguageServer::CompletionItemKinds::METHOD)
  end
  ...
  it "marks aliases as methods or attributes in definitions" do
    ...
    foo_alias = pins.find { |pin| pin.name == "foo_alias" }
    expect(foo_alias.symbol_kind).to eq(Lunargraph::LanguageServer::SymbolKinds::PROPERTY)
    bar_alias = pins.find { |pin| pin.name == "bar_alias" }
    expect(bar_alias.symbol_kind).to eq(Lunargraph::LanguageServer::SymbolKinds::METHOD)
  end
  ...
  describe "#locate_ref" do
    it "returns nil without a matching reference location" do
      ...
      library.map!
      location = Lunargraph::Location.new(File.join(workspace, "app.rb"), Lunargraph::Range.from_to(0, 8, 0, 8))
